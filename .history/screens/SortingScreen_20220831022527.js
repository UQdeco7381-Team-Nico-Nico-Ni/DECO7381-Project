...
const renderGridItem = (garbages) => {
    return garbages.map((garbage) => {
        return (
          <DragBox id={garbage.id} title={garbage.title} onDragDrop={() => deleteItem(id)}/>
        )
    })
};

const deleteItem = (id) => {
    setGarbages(garbages.filter(item => item.title !== "Papers"));
...
return (
    <DraxProvider>
      <View style={styles.palette}>
        ...
        {/* <DragBox id={garbages[0].id} title={GARBAGES[0].title} onDragDrop={() => deleteItem(id)}/>
